...
const Booking: React.FC = () => {
  ...
  const handleStripePayment = async () => {
    if (!window.Stripe) {
      setErrorMsg("Stripe gateway is currently unavailable. Please refresh and try again.");
      return;
    }

    setStep('processing');
    setProcessingStatus("Initializing Stripe Checkout...");

    try {
      // In a real app, you would use your Public Key here.
      // The Secret Key would be used on your server to create a Session.
      const stripe = window.Stripe(process.env.STRIPE_PUBLIC_KEY || 'pk_test_51P8K0eL6XpPlaceholder');

      // Step 1: Simulated server-side Session Creation
      await new Promise(r => setTimeout(r, 1200));
      setProcessingStatus("Redirecting to Secure Payment...");

      // Step 2: Simulated Redirection to Stripe-hosted page
      // Normally: const { error } = await stripe.redirectToCheckout({ sessionId: 'session_id_from_server' });
      ...
      setErrorMsg("Payment process interrupted. Please contact support or try again.");
      setStep('payment');
    }
  };
                ...
                <div role="row" className="contents">
                  {grid.map((day, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`w-12 h-12 flex items-center justify-center rounded-sm z-10 text-[13px] transition-all duration-300 ${
                          isStart || isEnd 
